...
export async function getRecentMessages(args: { topic: string; limit: number }) {
  ...
  const result = await pool.query<DbMessageRow>(
    `
      SELECT id, guest_id, topic_slug, text, created_at
      FROM messages
      WHERE topic_slug = $1
...
export async function insertMessage(args: { guestId: string; topic: string; text: string }) {
  const pool = getDbPool();
  const result = await pool.query<DbMessageRow>(
    `
      INSERT INTO messages (guest_id, topic_slug, text)
      VALUES ($1, $2, $3)
      RETURNING id, guest_id, topic_slug, text, created_at
    `,
    [args.guestId, args.topic, args.text],
  );
